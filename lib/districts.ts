...
// Her ilin ilçeleri alfabetik sırada

export interface DistrictData {
    [cityName: string]: string[];
}

export const districtsByCity: DistrictData = {
...
export const getDistrictsByCity = (cityName: string): string[] => {
    return districtsByCity[cityName] || [];
};

// Get all cities that have districts defined
export const getCitiesWithDistricts = (): string[] => {
    return Object.keys(districtsByCity).sort();
};

// Find district in a city (case-insensitive)
export const findDistrictInCity = (cityName: string, districtName: string): string | undefined => {
    const districts = districtsByCity[cityName];
    if (!districts) return undefined;
    return districts.find(d => d.toLowerCase() === districtName.toLowerCase());
};
